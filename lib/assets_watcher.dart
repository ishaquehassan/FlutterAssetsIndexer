...
void watchAssets(List<String> arguments) async {
...
Future<File> putClassCode(File myFile) {
  ...
  return myFile.writeAsString(
      classNameLine + '\n' + basePathLine + '\n\n' + closingLine);
}

typedef onFileCreated(dynamic file);

Future<dynamic> createAssetsFile(String path, {bool isDir = false}) async {
  var myFile;
  if (!isDir) {
    myFile = classFile;
  } else {
...
Future<File> removeAsset(String line) async {
  var index = lines.indexWhere((line) => line.contains(line));
  if (index >= 0) {
